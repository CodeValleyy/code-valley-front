<template>
  <v-container class="min-h-screen">
    ...
    <div v-else v-for="notification in notifications" :key="notification.id">
      <div
        class="mt-2 p-4 border rounded shadow-lg cursor"
        :class="{
          ...
          <v-menu>
            ...
            <v-list>
              <v-list-item @click="removeNotification(notification)">
                <v-list-item-title class="flex items-center text-red-500 text-xs"
                  >Supprimer</v-list-item-title
                >
              </v-list-item>
              <v-list-item v-if="notification.hasBeenRead" @click="unseeNotification(notification)">
                <v-list-item-title class="flex items-center text-blue-500 text-xs"
                  >Marquer comme non lu</v-list-item-title
                >
              </v-list-item>
            </v-list>
          </v-menu>
    ...
    <div class="mt-3 mb-3" v-if="notificationCount > limit">
      {{ notificationCount - limit }} notifications masqués
    </div>
  </v-container>
</template>
...
const acceptFriendRequest = async (notification: Notification) => {
  ...
  if (isSuccessful) {
    idsFriendshipRequestButtonShowed.value = idsFriendshipRequestButtonShowed.value.filter(
